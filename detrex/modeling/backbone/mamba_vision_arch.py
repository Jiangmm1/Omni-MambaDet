...
class MambaVisionMixer(nn.Module):
    """
    Modified MambaVisionMixer with SS2D (Cross-Scan) mechanism
    """
    ...
    def forward(self, hidden_states):
        ...
        # Infer H and W assuming square feature maps/windows
        H = W = int(math.sqrt(seqlen))

        xz = self.in_proj(hidden_states)  # (B, L, d_inner)

        # Split x and z
        # x is the signal for SSM, z is the gate
        x, z = xz.chunk(2, dim=-1)  # (B, L, d_inner/2)

        # ------------------------------------------------------------------
        # SS2D (Cross-Scan) Logic
        # ------------------------------------------------------------------
        # Helper to generate 4 scan directions
        def get_cross_scan_inputs(feat):
            # feat: (B, L, C)
            feat_2d = feat.view(B, H, W, -1)

            # Direction 1: Original (Row-Major)
            v1 = feat

            # Direction 2: Reverse Row-Major (Horizontal Flip + Flatten effectively? No, Flatten then Reverse)
            # Actually, standard Bi-Mamba reverses the sequence.
            # But true SS2D does spatial manipulation.
            # Let's use:
            # 1. Top-Left -> Bottom-Right (Original)
            # 2. Bottom-Right -> Top-Left (Original Flip)
            # 3. Top-Right -> Bottom-Left (Transpose)
            # 4. Bottom-Left -> Top-Right (Transpose Flip)

            v2 = feat.flip(1)

            # Transpose (B, W, H, C) -> Flatten
            v3 = feat_2d.transpose(1, 2).reshape(B, seqlen, -1)

            v4 = v3.flip(1)

            # Stack into batch dimension: (4*B, L, C)
            return torch.cat([v1, v2, v3, v4], dim=0)

        # Prepare 4x inputs
        x_4x = get_cross_scan_inputs(x)  # (4B, L, D')
        z_4x = get_cross_scan_inputs(z)  # (4B, L, D')

        # Rearrange for Conv1d (B, D, L)
        x_4x = rearrange(x_4x, "b l d -> b d l")
        z_4x = rearrange(z_4x, "b l d -> b d l")

        # Apply Conv1d
        x_4x = F.silu(F.conv1d(input=x_4x, weight=self.conv1d_x.weight, bias=self.conv1d_x.bias, padding='same',
                               groups=self.d_inner // 2))
        z_4x = F.silu(F.conv1d(input=z_4x, weight=self.conv1d_z.weight, bias=self.conv1d_z.bias, padding='same',
                               groups=self.d_inner // 2))

        # SSM Projection
        x_dbl = self.x_proj(rearrange(x_4x, "b d l -> (b l) d"))
        ...
        y_4x = selective_scan_fn(
            ...
            C_ssm,
            self.D.float(),
            z=None,  # We apply z later
            delta_bias=self.dt_proj.bias.float(),
            delta_softplus=True,
            return_last_state=None
        )  # (4B, D, L)

        # Rearrange back to (4B, L, D)
        y_4x = rearrange(y_4x, "b d l -> b l d")

        # ------------------------------------------------------------------
        # Cross-Merge (Sum) Logic
        # ------------------------------------------------------------------
        # Unpack the 4 directions
        y1, y2, y3, y4 = y_4x.chunk(4, dim=0)  # Each (B, L, D)

        # Restore directions
        # y1 is already correct
        y2 = y2.flip(1)
        # y3 was transposed
        y3 = y3.view(B, W, H, -1).transpose(1, 2).reshape(B, seqlen, -1)
        # y4 was transposed and flipped
        y4 = y4.flip(1).view(B, W, H, -1).transpose(1, 2).reshape(B, seqlen, -1)
        ...
        # ------------------------------------------------------------------
        # The original z (gate) corresponds to the original direction 'v1'
        # Since we summed up y, we should probably gate the summed y with the original z?
        # Or did we process z in 4 directions just for symmetry?
        # In original MambaVision, z is passed through conv1d and silu.
        # Here we did that for all 4 directions of z.
        # Let's take z1 (original direction) for the final gate to keep shapes aligned.

        z1, _, _, _ = z_4x.chunk(4, dim=0)  # (B, D, L)
        z1 = rearrange(z1, "b d l -> b l d")  # (B, L, D)

        # Original code uses concatenation for the output projection: y = torch.cat([y, z], dim=1)
        # But wait, dims don't match for cat dim=1 if shapes are (B, L, D).
        # Original code:
        #   xz shape was (B, D, L) after rearrange.
        #   x, z shape (B, D/2, L).
        #   y shape (B, D/2, L).
        #   cat([y, z], dim=1) -> (B, D, L).
        #   rearrange -> (B, L, D).
        ...
        y_combined = torch.cat([y, z1], dim=-1)  # (B, L, D_inner)
